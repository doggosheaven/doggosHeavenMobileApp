...
/**
 * Shared by /staff/billing and /admin/billing. The two copies differed only in
 * which billhistory route they navigate to, plus a couple of header pixels.
 */
export default function BillingScreen({ basePath, closeIconSize = 22, headerGap = false }) {
  ...
  const handleCash = async () => {
    ...
    Alert.alert(
      ...
      [{ text: "OK", onPress: () => { setForm(emptyForm()); router.push(`${basePath}/billhistory`); } }]
    );
  };
    ...
    try {
      ...
      if (msg.type === "success") {
        ...
        if (verifyData.success) {
          ...
          await saveBillToBackend(data);
          setForm(emptyForm());
          router.push(`${basePath}/billhistory`);
        } else {
  ...
  return (
    <View style={s.container}>
      <View style={[s.header, headerGap && s.headerGap]}>
        <TouchableOpacity onPress={() => router.back()} style={s.backBtn}>
          <Ionicons name="close" size={closeIconSize} color="#fff" />
        </TouchableOpacity>
        <Text style={[s.headerTitle, headerGap && s.headerTitleLeft]}>Walk-in Billing</Text>
        <TouchableOpacity onPress={() => router.push(`${basePath}/billhistory`)} style={s.historyBtn}>
